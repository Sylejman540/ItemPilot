...
try {
  ...
  // 3) If no other mappings in THIS table still use the same name, drop the column
  $stmt = $conn->prepare("
    SELECT COUNT(*)
      FROM applicants_fields
     WHERE user_id=? AND table_id=? AND field_name=?
  ");
