...
$tblStmt = $conn->prepare(
    "SELECT title FROM universal WHERE user_id = ? ORDER BY id ASC LIMIT 1"
);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
 ...
 if (isset($_FILES['attachment_summary']) && $_FILES['attachment_summary']['error'] === UPLOAD_ERR_OK) {
    ...
    // __DIR__ is already .../categories/Universal Table
    ...
    if (empty($id)) {
        $stmt = $conn->prepare(
            "INSERT INTO universal (name, notes, title, assignee, status, attachment_summary, user_id)
             VALUES (?, ?, ?, ?, ?, ?, ?)"
        );
...
<body>
  ...
  <header class="absolute md:w-[75%] md:ml-16 md:mr-16 w-full h-96 bg-white">
    <section class="md:flex py-4 md:justify-between justify-center ml-3" id="randomHeader">
      <?php if ($tableTitle): ?>
        <div class="px-4 py-2 text-center flex">
  ...
  <table class="md:w-full w-240 divide-y divide-gray-200 border-collapse border border-gray-300">
    <tbody class="divide-y divide-gray-200">
      <?php if ($hasRecord): foreach ($rows as $r): ?>
        <tr>
          <td class="border-l border-gray-300 px-4 py-2 text-sm text-gray-900"><?= htmlspecialchars($r['name']) ?></td>
          <td class="border-l border-gray-300 px-4 py-2 text-sm text-gray-700"><?= htmlspecialchars($r['notes']) ?></td>
          <td class="border-l border-gray-300 px-4 py-2 text-sm text-gray-900"><?= htmlspecialchars($r['assignee']) ?></td>
          <td class="border-l border-gray-300 px-4 py-2"><span class="px-2 py-1 text-xs bg-green-100 rounded"><?= htmlspecialchars($r['status']) ?></span></td>
          <td class="border-l border-gray-300 px-4 py-2 text-sm text-gray-500">
          ...
          <td class="border-l border-gray-300 px-2 py-4 text-left text-sm flex gap-5">
            ...
            <a id="openTheadForm" href="/ItemPilot/categories/Universal%20Table/edit_tbody.php?id=<?= $r['id'] ?>" class="inline-block px-2 py-1 text-blue-500 border border-blue-500 rounded hover:bg-blue-50 transition" aria-label="Edit">
            <!-- Pen Icon -->
            <svg xmlns="http://www.w3.org/2000/svg" class="inline h-4 w-4" fill="none" stroke="blue" stroke-width="1" stroke-linecap="round" stroke-linejoin="round" viewBox="0 0 24 24">
              <path d="M12 20h9" />
  ...
  <div class="bg-white w-full max-w-md p-8 rounded-2xl shadow-lg relative">
    ...
    <form  action="/ItemPilot/categories/Universal Table/edit_tbody.php?id=<?= $r['id'] ?>" method="post">
      ...
      <div>
        <label><?= htmlspecialchars($thead['thead_status'] ?? 'Status') ?></label>
        <select type="text" name="status" id="status" value="<?= htmlspecialchars($first['status']) ?>" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition">
          <option name="do" id="do">To Do</option>
          <option name="progress" id="progress">In Progress</option>
          <option name="done" id="done">Done</option>
        </select>
