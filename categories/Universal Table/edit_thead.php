...
try {
  ...
  $sql1 = "INSERT INTO universal_thead (thead_name, thead_notes, thead_assignee, thead_status, thead_attachment, user_id, table_id) VALUES (?, ?, ?, ?, ?, ?, ?)";
  $stmt1 = $conn->prepare($sql1);
  if (!$stmt1) { 
    throw new Exception('Prepare failed: ' . $conn->error); 
  }
  $stmt1->bind_param('sssssii', $thead_name, $thead_notes, $thead_assignee, $thead_status, $thead_attachment, $uid, $table_id);
  $stmt1->execute();
  $stmt1->close();

  $sql2 ="UPDATE universal_fields SET field_name = ? WHERE id = ? AND table_id = ? AND user_id = ?";
