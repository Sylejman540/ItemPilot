...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($rec_id <= 0) {
    // CREATE in `dresses`
    $stmt = $conn->prepare("INSERT INTO dresses (linked_initiatives, executive_sponsor, status, complete, notes, priority, owner, deadline, attachment, table_id, user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
    $stmt->bind_param('sssssssssii', $linked_initiatives, $executive_sponsor, $status, $complete, $notes, $priority, $owner, $deadline, $attachment, $table_id, $uid);
    $stmt->execute();
    $row_id = (int)$stmt->insert_id;
    $stmt->close();

    // ensure/create dresses_base
    if ($toSave) {
    ...
    // UPDATE in `dresses`
    $stmt = $conn->prepare("UPDATE dresses 
SET linked_initiatives = ?, executive_sponsor = ?, status = ?, complete = ?, notes = ?, priority = ?, owner = ?, deadline = ?, attachment = ? WHERE id = ? AND table_id = ? AND user_id = ?");
    $stmt->bind_param('sssssssssiii', $linked_initiatives, $executive_sponsor, $status, $complete, $notes, $priority, $owner, $deadline, $attachment, $rec_id, $table_id, $uid);
    $stmt->execute();
    $stmt->close();

    // ensure dresses_base row exists
  ...
  if (is_ajax()) {
    ...
    <form method="POST"
      ...
      <div class="p-2 text-gray-600" data-col="linked_initiatives">
        <input type="text" name="linked_initiatives" value="<?= htmlspecialchars($linked_initiatives, ENT_QUOTES) ?>"
      ...
      <div class="p-2 text-gray-600" data-col="executive_sponsor">
        <input type="text" name="executive_sponsor" value="<?= htmlspecialchars($executive_sponsor, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>
      ...
      <div class="p-2 text-gray-600" data-col="complete">
        <input type="text" name="complete" value="<?= htmlspecialchars($complete, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>
      ...
      <div class="p-2 text-gray-600" data-col="priority">
        <input type="text" name="priority" value="<?= htmlspecialchars($priority, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>

      <div class="p-2 text-gray-600" data-col="owner">
        <input type="text" name="owner" value="<?= htmlspecialchars($owner, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>

      <div class="p-2 text-gray-600" data-col="deadline">
        <input type="text" name="deadline" value="<?= htmlspecialchars($deadline, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>
...
// --------- Table title ----------
$stmt = $conn->prepare("SELECT table_title FROM dresses_table WHERE user_id = ? AND table_id = ? LIMIT 1");
...
$stmt = $conn->prepare("SELECT id, table_id, linked_initiatives, executive_sponsor, status, complete, notes, priority, owner, deadline, attachment FROM dresses_thead WHERE user_id = ? AND table_id = ? ORDER BY id ASC LIMIT 1");
$stmt->bind_param('ii', $uid, $table_id);
$stmt->execute();
...
if ($theadRes && $theadRes->num_rows) {
  ...
  $thead = [   'linked_initiatives' => 'Linked Initiatives',
  ...
  $ins = $conn->prepare("INSERT INTO dresses_thead (user_id, table_id, linked_initiatives, executive_sponsor, status, complete, notes, priority, owner, deadline, attachment) VALUES (?,?,?,?,?,?,?,?,?,?,?)");
  $ins->bind_param('iisssssssss', $uid, $table_id, $thead['linked_initiatives'], $thead['executive_sponsor'], $thead['status'], $thead['complete'], $thead['notes'], $thead['priority'], $thead['owner'], $thead['deadline'], $thead['attachment']);
...
$dataStmt = $conn->prepare("SELECT id, linked_initiatives, executive_sponsor, status, complete, notes, priority, owner, deadline, attachment FROM dresses WHERE user_id = ? AND table_id = ? ORDER BY id ASC LIMIT ? OFFSET ?");
...
$fixedCount = 9; $hasAction = true; $totalCols  = $fixedCount + $dynCount + ($hasAction ? 1 : 0);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<main class="md:mt-0 mt-10 overflow-x-auto md:overflow-x-hidden">
  ...
  <div class="px-5 py-4">
    <form action="<?= $CATEGORY_URL ?>/delete_fields.php" method="post" class="space-y-3">
      <input type="hidden" name="table_id" value="<?= (int)($table_id ?? 0) ?>">
      <?php
        $sql2 = "SELECT id, field_name FROM dresses_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
  ...
  <!-- THEAD (editable) -->
  <div class="dresses-table" id="ut-<?= (int)$table_id ?>" data-table-id="<?= (int)$table_id ?>">
    <form action="<?= $CATEGORY_URL ?>/edit_thead.php" method="post" class="w-full thead-form border-b border-gray-200" data-table-id="<?= (int)$table_id ?>">
      ...
      <div class="app-grid gap-2 text-xs font-semibold text-black uppercase" style="--cols: <?= (int)$totalCols ?>;">
        <div class="p-2"><input name="linked_initiatives" value="<?= htmlspecialchars($thead['linked_initiatives'] ?? 'Linked Initiatives', ENT_QUOTES) ?>" placeholder="Name" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="executive_sponsor" value="<?= htmlspecialchars($thead['executive_sponsor'] ?? 'Executive Sponsor', ENT_QUOTES) ?>" placeholder="Notes" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="status" value="<?= htmlspecialchars($thead['status'] ?? 'Status', ENT_QUOTES) ?>" placeholder="Assignee" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="complete" value="<?= htmlspecialchars($thead['complete'] ?? 'Complete', ENT_QUOTES) ?>" placeholder="Status" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="notes" value="<?= htmlspecialchars($thead['notes'] ?? 'Notes', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="priority" value="<?= htmlspecialchars($thead['priority'] ?? 'Priority', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="owner" value="<?= htmlspecialchars($thead['owner'] ?? 'Owner', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="deadline" value="<?= htmlspecialchars($thead['deadline'] ?? 'Deadline', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="attachment" value="<?= htmlspecialchars($thead['attachment'] ?? 'Attachment', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
  ...
  <?php if ($hasRecord): foreach ($rows as $r): ?>
    ...
    ?>
    <form method="POST" action="/ItemPilot/categories/Dresses/edit_tbody.php?id=<?= (int)$r['id'] ?>" enctype="multipart/form-data" class="universal-row border-b border-gray-200 hover:bg-gray-50 text-sm" style="--cols: <?= (int)$totalCols ?>;" data-status="<?= htmlspecialchars($r['status'] ?? '', ENT_QUOTES) ?>">
...
<div class="p-2 text-gray-600" data-col="linked_initiatives">
  <input type="text" name="linked_initiatives" 
         value="<?= htmlspecialchars($r['linked_initiatives'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>

<div class="p-2 text-gray-600" data-col="executive_sponsor">
  <input type="text" name="executive_sponsor" 
         value="<?= htmlspecialchars($r['executive_sponsor'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>
...
<div class="p-2 text-gray-600" data-col="complete">
  <input type="text" name="complete" 
         value="<?= htmlspecialchars($r['complete'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>
...
<div class="p-2 text-gray-600" data-col="priority">
  <input type="text" name="priority" 
         value="<?= htmlspecialchars($r['priority'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>

<div class="p-2 text-gray-600" data-col="owner">
  <input type="text" name="owner" 
         value="<?= htmlspecialchars($r['owner'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>

<div class="p-2 text-gray-600" data-col="deadline">
  <input type="text" readonly name="deadline" 
         value="<?= htmlspecialchars($r['deadline'] ?? '', ENT_QUOTES) ?>" 
         class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
</div>

<div class="p-2 text-gray-600" data-col="attachment">
  <?php if (!empty($r['attachment'])): ?>
    <?php $src = $UPLOAD_URL . '/' . rawurlencode($r['attachment']); ?>
    <img src="<?= htmlspecialchars($src, ENT_QUOTES) ?>" class="thumb" alt="Attachment">
  <?php else: ?>
    <span class="italic text-gray-400 ml-[5px]">📎 None</span>
  <?php endif; ?>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pagination my-4 flex justify-start md:justify-center space-x-2">
      <?php if ($page > 1): ?><a href="insert_dresses.php?page=<?= $page-1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">« Prev</a><?php endif; ?>
      <?php for ($i=1; $i<=$totalPages; $i++): ?>
        <a href="insert_dresses.php?page=<?= $i ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded transition <?= $i===$page ? 'bg-blue-600 text-white border-blue-600 font-semibold' : 'text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700' ?>"><?= $i ?></a>
      <?php endfor; ?>
      <?php if ($page < $totalPages): ?><a href="insert_dresses.php?page=<?= $page+1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">Next »</a><?php endif; ?>
    </div>
  <?php endif; ?>
</main>
</header>
...
<div id="addForm" class="min-h-screen flex items-center justify-center p-2 hidden relative mt-13">
  ...
  <label class="block text-sm font-medium text-gray-700 mb-1">
    <?= htmlspecialchars($thead['linked_initiatives'] ?? 'Linked Initiatives') ?>
  </label>
  <input type="text" name="linked_initiatives" 
         placeholder="<?= htmlspecialchars($thead['linked_initiatives'] ?? 'Linked Initiatives') ?>" 
         class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
</div>

<div>
  <label class="block text-sm font-medium text-gray-700 mb-1">
    <?= htmlspecialchars($thead['executive_sponsor'] ?? 'Executive Sponsor') ?>
  </label>
  <input type="text" name="executive_sponsor" 
         placeholder="<?= htmlspecialchars($thead['executive_sponsor'] ?? 'Executive Sponsor') ?>" 
         class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
</div>
...
<div>
  <label class="block text-sm font-medium text-gray-700 mb-1">
    <?= htmlspecialchars($thead['complete'] ?? 'Complete') ?>
  </label>
  <input type="text" name="complete" 
         placeholder="<?= htmlspecialchars($thead['complete'] ?? 'Complete') ?>" 
         class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
</div>
...
<div>
  <label class="block text-sm font-medium text-gray-700 mb-1">
    <?= htmlspecialchars($thead['priority'] ?? 'Priority') ?>
  </label>
  <input type="text" name="priority" 
         placeholder="<?= htmlspecialchars($thead['priority'] ?? 'Priority') ?>" 
         class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
</div>

<div>
  <label class="block text-sm font-medium text-gray-700 mb-1">
    <?= htmlspecialchars($thead['owner'] ?? 'Owner') ?>
  </label>
  <input type="text" name="owner" 
         placeholder="<?= htmlspecialchars($thead['owner'] ?? 'Owner') ?>" 
         class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
</div>
...
<div>
  ...
  <input id="attachment" type="file" name="attachment" accept="image/*"
      ...
      <?php
        $sql3 = "SELECT id, field_name FROM dresses_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
