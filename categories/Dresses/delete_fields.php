...
try {
  ...
  if ($table_id <= 0) {
    $stmt = $conn->prepare("SELECT table_id FROM dresses_fields WHERE id=? AND user_id=? LIMIT 1");
  ...
  // 3) Drop column in dresses_base if no other mapping references it
  ...
  if ((int)$stillUsing === 0) {
    ...
    if ($exists) {
      $conn->query("ALTER TABLE `dresses_base` DROP COLUMN `{$colName}`");
      $dropped = true;
    }
  }
