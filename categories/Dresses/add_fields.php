...
$existsQ = $conn->prepare("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='dresses_base' AND COLUMN_NAME=?");
...
/* ---------- add column to dresses_base if missing ---------- */
$alterSql = "ALTER TABLE dresses_base ADD COLUMN `$col` VARCHAR(255) NULL";
if (!$conn->query($alterSql)) {
  // If it already exists due to race, ignore; otherwise error
  if (stripos($conn->error, 'Duplicate column name') === false) {
    json_out(['ok'=>false, 'error'=>'Failed to alter dresses_base: '.$conn->error], 500);
  }
}

/* ---------- insert into dresses_fields (metadata) ---------- */
$ins = $conn->prepare("INSERT INTO dresses_fields (user_id, table_id, field_name) VALUES (?,?,?)");
