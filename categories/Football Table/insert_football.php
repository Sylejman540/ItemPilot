...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (is_ajax()) {
    ...
    $fieldsAjax = [];
    $stmtF = $conn->prepare("SELECT id, field_name FROM football_fields WHERE user_id = ? AND table_id = ? ORDER BY id  ASC");
    ...
    ?>
    <form method="POST"
          action="/ItemPilot/categories/Football%20Table/edit_tbody.php?id=<?= (int)$row_id ?>"
...
      <?php
      ...
      <div class="p-2 text-gray-600" data-col="home_address">
        <input type="text" name="home_address" value="<?= htmlspecialchars($home_address, ENT_QUOTES) ?>"
      ...
      <div class="p-2 text-gray-600" data-col="email_address">
        <input type="text" name="email_address" value="<?= htmlspecialchars($email_address, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>
      ...
      <div class="p-2 text-gray-600" data-col="dyn">
        <?php
          ...
          $baseRowAjax = [];
          $stmtX = $conn->prepare("SELECT * FROM football_base WHERE table_id=? AND user_id=? AND row_id=? LIMIT 1");
...
$stmt = $conn->prepare("SELECT id, table_id, photo, full_name, position, home_address, email_address, notes  FROM football_thead WHERE user_id = ? AND table_id = ? ORDER BY id ASC LIMIT 1");
$stmt->bind_param('ii', $uid, $table_id);
$stmt->execute();
...
if ($theadRes && $theadRes->num_rows) {
  ...
  $thead = [ 'photo' => 'Photo', 'full_name' => 'Name', 'position' => 'Position', 'home_address' => 'Home Address', 'email_address' => 'Email Address', 'notes' => 'Notes' ];
  $ins = $conn->prepare("INSERT INTO football_thead (user_id, table_id,   photo, full_name, position, home_address, email_address, notes) VALUES (?,?,?,?,?,?,?,?)");
  $ins->bind_param('iissssss', $uid, $table_id, $thead['photo'], $thead['full_name'], $thead['position'], $thead['home_address'], $thead['email_address'], $thead['notes']);
...
<main class="md:mt-0 mt-10 overflow-x-auto md:overflow-x-hidden">
  ...
  <div class="px-5 py-4">
    <form action="<?= $CATEGORY_URL ?>/delete_fields.php" method="post" class="space-y-3">
      <input type="hidden" name="table_id" value="<?= (int)($table_id ?? 0) ?>">
      <?php
        $sql2 = "SELECT id, field_name FROM football_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
  ...
  <!-- THEAD (editable) -->
  <div class="football-table" id="ut-<?= (int)$table_id ?>" data-table-id="<?= (int)$table_id ?>">
    <form action="<?= $CATEGORY_URL ?>/edit_thead.php" method="post" class="w-full thead-form border-b border-gray-200" data-table-id="<?= (int)$table_id ?>">
      ...
      <div class="app-grid gap-2 text-xs font-semibold text-black uppercase" style="--cols: <?= (int)$totalCols ?>;">
        ...
        <div class="p-2"><input name="full_name" value="<?= htmlspecialchars($thead['full_name'] ?? 'Name', ENT_QUOTES) ?>" placeholder="Notes" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="position" value="<?= htmlspecialchars($thead['position'] ?? 'Position', ENT_QUOTES) ?>" placeholder="Assignee" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="home_address" value="<?= htmlspecialchars($thead['home_address'] ?? 'Home Address', ENT_QUOTES) ?>" placeholder="Status" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="email_address" value="<?= htmlspecialchars($thead['email_address'] ?? 'Email Address', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
  ...
  <?php if ($hasRecord): foreach ($rows as $r): ?>
    ...
    <form method="POST" action="/ItemPilot/categories/Football%20Table/edit_tbody.php?id=<?= (int)$r['id'] ?>" enctype="multipart/form-data" class="football-row border-b border-gray-200 hover:bg-gray-50 text-sm" style="--cols: <?= (int)$totalCols ?>;" data-status="<?= htmlspecialchars($r['status'] ?? '', ENT_QUOTES) ?>">
      ...
      <!-- Full Name -->
      <div class="p-2 text-gray-600" data-col="full_name">
        <input type="text" name="full_name" value="<?= htmlspecialchars($r['full_name'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
               class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
      </div>
      ...
      <!-- Home Address -->
      <div class="p-2 text-gray-600" data-col="home_address">
        <input type="text" name="home_address" value="<?= htmlspecialchars($r['home_address'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
               class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline:none focus:ring-2 focus:ring-blue-400 transition" />
      </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pagination my-4 flex justify-start md:justify-center space-x-2">
      <?php if ($page > 1): ?><a href="insert_football.php?page=<?= $page-1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">« Prev</a><?php endif; ?>
      ...
      <?php endfor; ?>
      <?php if ($page < $totalPages): ?><a href="insert_football.php?page=<?= $page+1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">Next »</a><?php endif; ?>
    </div>
  <?php endif; ?>
</main>
</header>
...
<div id="addForm" class="min-h-screen flex items-center justify-center p-2 hidden relative mt-13">
  <div class="bg-white w-full max-w-md p-5 rounded-2xl shadow-lg" id="signup">
    ...
    <form action="<?= $CATEGORY_URL ?>/insert_football.php" method="POST" enctype="multipart/form-data" class="new-record-form space-y-6">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['full_name'] ?? 'Name') ?></label>
        <input type="text" name="full_name" placeholder="<?= htmlspecialchars($thead['full_name'] ?? 'Name') ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['position'] ?? 'Position', ENT_QUOTES, 'UTF-8') ?></label>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['home_address'] ?? 'Home Address') ?></label>
        <input type="text" name="home_address" placeholder="<?= htmlspecialchars($thead['home_address'] ?? 'Home Address') ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
      </div>
      ...
      <?php
        $sql3 = "SELECT id, field_name FROM football_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
