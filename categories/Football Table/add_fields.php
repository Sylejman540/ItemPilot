...
$existsQ = $conn->prepare("SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME='football_base' AND COLUMN_NAME=?");
...
/* ---------- add column to football_base if missing ---------- */
$alterSql = "ALTER TABLE football_base ADD COLUMN `$col` VARCHAR(255) NULL";
if (!$conn->query($alterSql)) {
  // If it already exists due to race, ignore; otherwise error
  if (stripos($conn->error, 'Duplicate column name') === false) {
    json_out(['ok'=>false, 'error'=>'Failed to alter football_base: '.$conn->error], 500);
  }
}

/* ---------- insert into football_fields (metadata) ---------- */
$ins = $conn->prepare("INSERT INTO football_fields (user_id, table_id, field_name) VALUES (?,?,?)");
