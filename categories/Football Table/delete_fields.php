...
try {
  ...
  if ($table_id <= 0) {
    $stmt = $conn->prepare("SELECT table_id FROM footballl_fields WHERE id=? AND user_id=? LIMIT 1");
  ...
  $stmt = $conn->prepare("SELECT field_name FROM football_fields WHERE id=? AND table_id=? AND user_id=? LIMIT 1");
  ...
  $stmt = $conn->prepare("DELETE FROM football_fields WHERE id=? AND table_id=? AND user_id=?");
  $stmt->bind_param('iii', $id, $table_id, $uid);
  $stmt->execute();
  if ($stmt->affected_rows === 0) throw new RuntimeException("Delete failed or already removed.");
  $stmt->close();

  // 3) Drop column in fotoball_base if no other mapping references it
  $stmt = $conn->prepare("SELECT COUNT(*) FROM football_fields WHERE user_id=? AND table_id=? AND field_name=?");
  ...
  if ((int)$stillUsing === 0) {
    ...
    if ($exists) {
      $conn->query("ALTER TABLE `football_base` DROP COLUMN `{$colName}`");
      $dropped = true;
    }
  }
