...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($rec_id <= 0) {
    ...
    $stmt = $conn->prepare("INSERT INTO groceries (brand_flavor, quantity, department, purchased, notes, photo, table_id, user_id) VALUES (?,?,?,?,?,?,?,?)");
    $stmt->bind_param('sssissii', $brand_flavor, $quantity, $department, $purchased, $notes, $photo, $table_id, $uid);
    $stmt->execute();
    ...
    // UPDATE groceries
    $stmt = $conn->prepare("UPDATE groceries SET brand_flavor=?, quantity=?, department=?, purchased=?, notes=?, photo=? WHERE id=? AND table_id=? AND user_id=?");
    $stmt->bind_param('sssissiii', $brand_flavor, $quantity, $department, $purchased, $notes, $photo, $rec_id, $table_id, $uid);
    $stmt->execute();
    $stmt->close();

    // ensure groceries_base exists
...
<header id="appHeader" class="absolute md:mt-13 mt-20 transition-all duration-300 ease-in-out" style="padding-left:1.25rem;padding-right:1.25rem;">
  <section class="rename-table-form flex mt-6 justify-between ml-3 rename-table-form">
  ...
  <!-- Body -->
  <div class="px-5 py-4">
    <form action="/ItemPilot/categories/Groceries%20Table/add_fields.php" method="post" class="space-y-3">
  ...
  <!-- Body -->
  <div class="px-5 py-4">
    <form action="/ItemPilot/categories/Groceries%20Table/delete_fields.php" method="post" class="space-y-3">
      <input type="hidden" name="table_id" value="<?= (int)($table_id ?? 0) ?>">
      ...
      <div class="divide-y divide-gray-100 rounded-xl overflow-hidden ring-1 ring-gray-100">
        <?php foreach ($fields as $field): ?>
          <div class="flex items-center justify-between gap-2 px-3 py-2 hover:bg-gray-50 transition">
            ...
            <a href="/ItemPilot/categories/Groceries%20Table/delete_fields.php?id=<?= (int)$field['id'] ?>&table_id=<?= (int)$table_id ?>"
...
<div class="groceries-table" id="gt-<?= (int)$table_id ?>" data-table-id="<?= (int)$table_id ?>">
  <form action="<?= $CATEGORY_URL ?>/edit_thead.php" method="post"
        class="w-full thead-form border-b border-gray-200 app-grid gap-2 text-xs font-semibold text-black uppercase"
        style="--cols: <?= (int)$totalCols ?>;"
        data-table-id="<?= (int)$table_id ?>">
    <input type="hidden" name="table_id" value="<?= (int)$table_id ?>">

    <div class="p-2">
      <input name="photo" value="<?= htmlspecialchars($head['photo'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Photo"
    ...
    <div class="p-2">
      <input name="brand_flavor" value="<?= htmlspecialchars($head['brand_flavor'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Brand/Flavor"
             class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
    </div>
    <div class="p-2">
      <input name="quantity" value="<?= htmlspecialchars($head['quantity'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Quantity"
             class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
    </div>
    <div class="p-2">
      <input name="department" value="<?= htmlspecialchars($head['department'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Department"
             class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
    </div>
    <div class="p-2">
      <input name="purchased" value="<?= htmlspecialchars($head['purchased'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Purchased"
             class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
    </div>
    <div class="p-2">
      <input name="notes" value="<?= htmlspecialchars($head['notes'], ENT_QUOTES, 'UTF-8') ?>"
             placeholder="Notes"
             class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
    </div>
      ...
      <div class="p-2 text-gray-600" data-col="photo">
        <?php if (!empty($r['photo'])): ?>
          <img src="<?= $UPLOAD_URL . '/' . rawurlencode($r['photo']) ?>" class="thumb"
               alt="<?= htmlspecialchars($r['brand_flavor'] ?? 'Attachment', ENT_QUOTES, 'UTF-8') ?>">
        <?php else: ?>
