...
/* ---------- Config (UNIVERSAL) ---------- */
$CATEGORY_URL = '/ItemPilot/categories/Groceries%20Table';
...
if ($action === 'create_blank') {
  ...
  if ($table_id <= 0) {
    $q = $conn->prepare("SELECT table_id FROM groceries_table WHERE user_id = ? ORDER BY table_id ASC LIMIT 1");
    $q->bind_param('i', $uid);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if (!empty($_FILES['photo']) && $_FILES['photo']['error'] !== UPLOAD_ERR_NO_FILE) {
    if ($_FILES['photo']['error'] !== UPLOAD_ERR_OK) {
      ...
      header("Location: /ItemPilot/home.php?autoload=1&type=groceries&table_id={$table_id}", true, 303);
      exit;
    }
    ...
    $photo = $fname;  // store just the filename in DB
  }
  ...
  if ($rec_id <= 0) {
    ...
    $stmt->bind_param('ssssssii', $photo, $brand_flavor, $quantity, $department, $purchased, $notes, $table_id, $uid);
    $stmt->execute();
    $row_id = (int)$stmt->insert_id;
    $stmt->close();

    // ensure/create groceries_base
    if ($toSave) {
    ...
    $stmt->bind_param('ssssssiii', $photo, $brand_flavor, $quantity, $department, $purchased, $notes, $rec_id, $table_id, $uid);
    $stmt->execute();
    $stmt->close();
  ...
  if (is_ajax()) {
    ...
    $fieldsAjax = [];
    $stmtF = $conn->prepare("SELECT id, field_name FROM groceries_fields WHERE user_id = ? AND table_id = ? ORDER BY id  ASC");
    ...
    <form method="POST"
      ...
      <input type="hidden" name="photo" value="<?= htmlspecialchars($photo, ENT_QUOTES) ?>">

      <div class="p-2 text-gray-600" data-col="photo">
        <?php if (!empty($photo)): ?>
          <?php $src = $UPLOAD_URL . '/' . rawurlencode($photo); ?>
      ...
      <div class="p-2 text-gray-600" data-col="brand_flavor">
        <input type="text" name="brand_flavor" value="<?= htmlspecialchars($brand_flavor, ENT_QUOTES) ?>"
      ...
      <div class="p-2 text-gray-600" data-col="quantity">
        <input type="text" name="quantity" value="<?= htmlspecialchars($quantity, ENT_QUOTES) ?>"
              class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/>
      </div>
...
       <?php
      ...
      <div class="p-2 text-gray-600" data-col="dyn">
        <?php
          ...
          $baseRowAjax = [];
          $stmtX = $conn->prepare("SELECT * FROM groceries_base WHERE table_id=? AND user_id=? AND row_id=? LIMIT 1");
...
$stmt = $conn->prepare("SELECT id, table_id, photo, brand_flavor, quantity, department, purchased, notes  FROM groceries_head WHERE user_id = ? AND table_id = ? ORDER BY id ASC LIMIT 1");
$stmt->bind_param('ii', $uid, $table_id);
$stmt->execute();
...
if ($theadRes && $theadRes->num_rows) {
  ...
  $thead = [ 'photo' => 'Photo', 'brand_flavor' => 'Brand Flavor', 'quantity' => 'Quantity', 'department' => 'Department', 'purchased' => 'Purchased', 'notes' => 'Notes' ];
  $ins = $conn->prepare("INSERT INTO groceries_head (user_id, table_id, photo, brand_flavor, quantity, department, purchased, notes) VALUES (?,?,?,?,?,?,?,?)");
  $ins->bind_param('iissssss', $uid, $table_id, $thead['photo'], $thead['brand_flavor'], $thead['quantity'], $thead['department'], $thead['purchased'], $thead['notes']);
...
$fixedCount = 6; $hasAction = true; $totalCols  = $fixedCount + $dynCount + ($hasAction ? 1 : 0);
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
<main class="md:mt-0 mt-10 overflow-x-auto md:overflow-x-hidden">
  <div class="mx-auto mt-12 mb-2 mr-5 bg-white p-4 md:p-8 lg:p-10 rounded-xl shadow-md border border-gray-100 md:w-full w-[120rem]">
  ...
  <div class="px-5 py-4">
    <form action="<?= $CATEGORY_URL ?>/delete_fields.php" method="post" class="space-y-3">
      <input type="hidden" name="table_id" value="<?= (int)($table_id ?? 0) ?>">
      <?php
        $sql2 = "SELECT id, field_name FROM groceries_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
  ...
  <div class="universal-table" id="ut-<?= (int)$table_id ?>" data-table-id="<?= (int)$table_id ?>">
    <form action="<?= $CATEGORY_URL ?>/edit_thead.php" method="post" class="w-full thead-form border-b border-gray-200" data-table-id="<?= (int)$table_id ?>">
      ...
      <div class="app-grid gap-2 text-xs font-semibold text-black uppercase" style="--cols: <?= (int)$totalCols ?>;">
        <div class="p-2"><input name="photo" value="<?= htmlspecialchars($thead['photo'] ?? 'Photo', ENT_QUOTES) ?>" placeholder="Name" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="brand_flavor" value="<?= htmlspecialchars($thead['brand_flavor'] ?? 'Brand Flavor', ENT_QUOTES) ?>" placeholder="Notes" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="quantity" value="<?= htmlspecialchars($thead['quantity'] ?? 'Quantity', ENT_QUOTES) ?>" placeholder="Assignee" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="department" value="<?= htmlspecialchars($thead['department'] ?? 'Department', ENT_QUOTES) ?>" placeholder="Status" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
        <div class="p-2"><input name="purchased" value="<?= htmlspecialchars($thead['purchased'] ?? 'Purchased', ENT_QUOTES) ?>" placeholder="Attachment" class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition"/></div>
  ...
  <?php if ($hasRecord): foreach ($rows as $r): ?>
    ...
    <form method="POST" action="/ItemPilot/categories/Groceries%20Table/edit_tbody.php?id=<?= (int)$r['id'] ?>" enctype="multipart/form-data" class="groceries-row border-b border-gray-200 hover:bg-gray-50 text-sm" style="--cols: <?= (int)$totalCols ?>;" data-status="<?= htmlspecialchars($r['status'] ?? '', ENT_QUOTES) ?>">
      <input type="hidden" name="id" value="<?= (int)$r['id'] ?>">
      <input type="hidden" name="table_id" value="<?= (int)$table_id ?>">
      <input type="hidden" name="photo" value="<?= htmlspecialchars($r['photo'] ?? '', ENT_QUOTES) ?>">

      <div class="p-2 text-gray-600" data-col="photo">
        <?php if (!empty($r['photo'])): ?>
          <img src="<?= $UPLOAD_URL . '/' . rawurlencode($r['photo']) ?>" class="thumb"
               alt="<?= htmlspecialchars($r['photo'] ?? 'Attachment', ENT_QUOTES, 'UTF-8') ?>">
        <?php else: ?>
      ...
      <!-- Brand/Flavor -->
      <div class="p-2 text-gray-600" data-col="brand_flavor">
        <input type="text" name="brand_flavor" value="<?= htmlspecialchars($r['brand_flavor'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
               class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 transition" />
      </div>

      <!-- Quantity -->
      <div class="p-2 text-gray-600" data-col="quantity">
        <input type="text" name="quantity" value="<?= htmlspecialchars($r['quantity'] ?? '', ENT_QUOTES, 'UTF-8') ?>"
               class="w-full bg-transparent border-none px-4 py-2 rounded-lg focus:outline:none focus:ring-2 focus:ring-blue-400 transition" />
      </div>
  ...
  <?php if ($totalPages > 1): ?>
    <div class="pagination my-4 flex justify-start md:justify-center space-x-2">
      <?php if ($page > 1): ?><a href="insert_groceries.php?page=<?= $page-1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">« Prev</a><?php endif; ?>
      ...
      <?php endfor; ?>
      <?php if ($page < $totalPages): ?><a href="insert_groceries.php?page=<?= $page+1 ?>&table_id=<?= (int)$table_id ?>" class="px-3 py-1 border rounded text-blue-600 border-blue-300 hover:bg-blue-50 hover:text-blue-700 transition">Next »</a><?php endif; ?>
    </div>
  <?php endif; ?>
</main>
</header>
...
<div id="addForm" class="min-h-screen flex items-center justify-center p-2 hidden relative mt-13">
  <div class="bg-white w-full max-w-md p-5 rounded-2xl shadow-lg" id="signup">
    ...
    <form action="<?= $CATEGORY_URL ?>/insert_groceries.php" method="POST" enctype="multipart/form-data" class="new-record-form space-y-6">
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['photo'] ?? 'Photo') ?></label>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['brand_flavor'] ?? 'Brand Flavor') ?></label>
        <input type="text" name="brand_flavor" placeholder="<?= htmlspecialchars($thead['brand_flavor'] ?? 'Brand Flavor') ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
      </div>
      
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['quantity'] ?? 'Quantity') ?></label>
        <input type="text" name="quantity" placeholder="<?= htmlspecialchars($thead['quantity'] ?? 'Quantity') ?>" class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"/>
      </div>

       <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['department'], ENT_QUOTES, 'UTF-8') ?></label>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 mb-1"><?= htmlspecialchars($thead['purchased'], ENT_QUOTES, 'UTF-8') ?></label>
        <label class="inline-flex items-center gap-2">
          <input type="checkbox" name="purchased" value="1" />
          <span><?= htmlspecialchars($thead['purchased'], ENT_QUOTES, 'UTF-8') ?></span>
        </label>
      </div>
      ...
      <?php
        $sql3 = "SELECT id, field_name FROM groceries_fields WHERE user_id = ? AND table_id = ? ORDER BY id ASC";
