...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  if ($field_name === '' || !$table_id || $uid <= 0) {
    header('Location: /ItemPilot/categories/Groceries%20Table/insert_groceries.php?error=missing_params');
    exit();
  }

  $sql = "INSERT INTO groceries_fields (table_id, user_id, field_name) VALUES (?, ?, ?)";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param('iis', $table_id, $uid, $field_name);
  $stmt->execute();

  // 1) Add the column (DDL → use query, not prepare)
  $conn->query("ALTER TABLE `groceries_base` ADD COLUMN `".str_replace('`','``',$field_name)."` TEXT");
